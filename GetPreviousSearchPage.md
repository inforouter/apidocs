...
- **GET** `/srv.asmx/GetPreviousSearchPage?authenticationTicket=...&withrules=...&withPropertySets=...&withSecurity=...&withOwner=...&withVersions=...`
- **POST** `/srv.asmx/GetPreviousSearchPage` (form data)
- **SOAP** Action: `http://tempuri.org/GetPreviousSearchPage`
...
- `GetPreviousSearchPage` must be called **after** a successful `Search` call and at least one `GetNextSearchPage` call. Calling it when already on the first page returns the first page again (`FirstPage="true"`).
...
- The cursor position is shared with `GetNextSearchPage` — both APIs operate on the same server-side pagination state for the current session.
- When already on the first page, `GetPreviousSearchPage` resets the cursor to position 1 and returns the first page again rather than returning an error.
...
- The parameters and response structure are identical to `GetNextSearchPage` — only the direction of cursor movement differs.

---

## Related APIs

- [Search](Search) - Prepare a search result set (must be called before this API)
- [GetNextSearchPage](GetNextSearchPage) - Retrieve the next page of the prepared search results
- [GetFoldersAndDocuments](GetFoldersAndDocuments) - List documents and folders in a specific path without a search query
- [GetDocuments](GetDocuments) - Return documents in a specific folder path
