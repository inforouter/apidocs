...
Returns the full-text abstract (indexed text content) of a specified version of a document. This is the current preferred API for retrieving document abstracts. It uses a consistent XML response structure for both success and error cases, unlike the obsolete [`GetDocumentAbstract`](GetDocumentAbstract).
...
Both success and error use the same `<response>` root element — the only difference is the presence of the `<Value>` child on success. This consistent structure is what makes `GetDocumentAbstract1` the preferred API over the obsolete [`GetDocumentAbstract`](GetDocumentAbstract).
...
- This API supersedes the obsolete [`GetDocumentAbstract`](GetDocumentAbstract), which used an `<abstract>` child element on success and had inconsistent XML structure between success and error cases.

---

## Related APIs

- [GetDocumentAbstract](GetDocumentAbstract) - Obsolete predecessor with inconsistent XML structure
- [GetDocument](GetDocument) - Get full document metadata and properties
- [GetDocumentVersions](GetDocumentVersions) - Get the version history list for a document
...
*For detailed documentation visit: https://support.inforouter.com/api-docs/GetDocumentAbstract1*
