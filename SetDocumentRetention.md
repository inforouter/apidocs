...
> **Obsolete since infoRouter 8.1.155.** This API always returns an error and must not be used in new integrations. Use [`SetDocumentRandDSchedule`](SetDocumentRandDSchedule) to manage document retention.
...
- Do not use this API in any new or existing integration. Migrate to [`SetDocumentRandDSchedule`](SetDocumentRandDSchedule), which applies a named Retention & Disposition schedule to a document.
...
- [SetDocumentRandDSchedule](SetDocumentRandDSchedule) - Assign a Retention & Disposition schedule to a document (replacement for this API)
- [GetDocumentRandDSchedule](GetDocumentRandDSchedule) - Get the current R&D schedule assigned to a document
- [RemoveDocumentRandDSchedule](RemoveDocumentRandDSchedule) - Remove the R&D schedule from a document
- [GetRandDSchedules](GetRandDSchedules) - List all defined R&D schedule definitions
