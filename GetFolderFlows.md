...
Step definitions are **not** included in the response. Use [GetFlowDef](GetFlowDef) to retrieve the full definition with steps and tasks for a specific workflow.
...
- Step and task definitions are **not** included. To get the full workflow definition with all steps and tasks, call [GetFlowDef](GetFlowDef) with the `FlowName` and `DomainName` from the response.
...
- To list all workflows for a domain regardless of folder, use [GetDomainFlows](GetDomainFlows).

## Related APIs

- [GetDomainFlows](GetDomainFlows) – List all workflow definitions for a domain.
- [GetFlowDef](GetFlowDef) – Get the full definition of a single workflow including steps and tasks.
- [SubmitDocumentToFlow](SubmitDocumentToFlow) – Submit a document to a workflow.
- [CreateFlowDef](CreateFlowDef) – Create a new workflow definition.
