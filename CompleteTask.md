...
- [GetTask](GetTask) – Retrieve task details including current status and requirements.
- [getTasks](getTasks) – Get a filtered list of tasks.
- [TestTaskCompletion](TestTaskCompletion) – Test whether all requirements are met before calling CompleteTask.
- [ReassignTask](ReassignTask) – Transfer a task to a different user before completing.
- [SetTaskComment](SetTaskComment) – Set the task comment without completing the task.
- [SetTaskApprovalStatus](SetTaskApprovalStatus) – Set the approval status of a task (Approve/Reject/No status).
- [ChangeTaskDueDate](ChangeTaskDueDate) – Extend or move the task due date.
