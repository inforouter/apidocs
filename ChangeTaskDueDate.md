...
- [GetTask](GetTask) – Retrieve the details of a specific task including its current status and due date.
- [getTasks](getTasks) – Get a filtered list of tasks to find task IDs.
- [ReassignTask](ReassignTask) – Reassign a task to a different user, optionally with a new due date.
- [UpdateTaskFinishDate](UpdateTaskFinishDate) – Update the recorded finish date of a completed task.
- [SetTaskPriority](SetTaskPriority) – Change the priority of a task.
