...
Reads the indexed text of one document version, in an `<abstract>` element.

```javascript
const root = await call('GetDocumentAbstract', {
  authenticationTicket: ticket,
  Path: '/Finance/Reports/Q1.pdf',
  VersionNumber: 0            // 0 means the published version
});

const element = root.querySelector('abstract');
console.log(element.textContent,
            element.getAttribute('AppliedBy'),
            element.getAttribute('DateApplied'));
```
