...
Reports the SOX compliance entries on one document. Entries are under `<Value>`, and a document with none is a success with nothing in
it rather than an error.

```javascript
const root = await call('GetSoxLogs', {
