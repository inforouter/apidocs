...
Moves the folders one user owns to another user.

```javascript
const root = await call('TransferUserFolderOwnerships', {
...
An answer may carry a `warnings` attribute when some of the folder ownerships could not be moved - a conflict at
