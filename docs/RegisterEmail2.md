...
Files an e-mail as a document, taking the folder and the document name separately. **This is the
variant to reach for**: the others take a single path that is the document's, not the folder's, which
is easy to get wrong.

```javascript
const root = await call('RegisterEmail2', {
  authenticationTicket: ticket,
  FolderPath: '/Finance/Emails',
  EmailName: '2026-09-supplier-query',       // .email is added
...
The addresses are stored on the document as a `SYSTBL_EMAIL` property set, readable with
[GetDocument](GetDocument.md) and `withPropertySets=true`. This variant stores `CCAddress` and
`BCCAddress` correctly; [RegisterEmail3](RegisterEmail3.md) does not.
...
- [RegisterEmail](RegisterEmail.md) - Register an email using a combined `TargetPath` (no keywords)
