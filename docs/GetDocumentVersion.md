...
| `versionNumber` | int | Yes | Version number to retrieve. Pass `0` to retrieve the currently published version. Otherwise use the modern format: version numbers pack a major, a minor and a revision into one integer as `major * 1000000 + minor * 1000 + revision`, so the first version is `1000000` and the second is `1000001` - not `2000000`, which would be major version 2. Values between `1` and `999,999` will not match any version and result in an error. |

### Version Number Format

infoRouter stores and returns version numbers in a large-integer format where version numbers pack a major, a minor and a revision into one integer as `major * 1000000 + minor * 1000 + revision`, so the first version is `1000000` and the second is `1000001` - not `2000000`, which would be major version 2. The `Number` attribute in the response always uses this format.
...
Reads one version of a document, in the same `<Version>` shape
[GetDocumentVersions](GetDocumentVersions.md) lists.

```javascript
const root = await call('GetDocumentVersion', {
...
console.log(version.getAttribute('Number'), version.querySelector('CheckSum').textContent);
```

A version number nothing uses is answered `4041`, "the specified version cannot be found". Worth
noting alongside [DeleteDocumentVersion](DeleteDocumentVersion.md), which answers the identical
condition with `4000` and a message about the file being in use - this operation is the one that
