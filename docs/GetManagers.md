...
Lists the managers of a library.

```javascript
const root = await call('GetManagers', { authenticationTicket: ticket, DomainName: 'Finance' });
...
A library with no managers answers with an empty `<users>` section rather than an error.
