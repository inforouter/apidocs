...
Reports what changed hands in a date range. Rows come back as `<LOGITEM>` inside `<logs>`.
...
```javascript
const root = await call('GetOwnershipChangeLog', {
...
for (const row of root.querySelectorAll('logs > LOGITEM')) {
