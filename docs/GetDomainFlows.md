...
Lists the workflow definitions belonging to a library.

```javascript
const root = await call('GetDomainFlows', { authenticationTicket: ticket, DomainName: 'Finance' });

for (const flow of root.querySelectorAll('FlowDefs > FlowDef')) {
  console.log(flow.getAttribute('FlowDefId'), flow.getAttribute('FlowName'));
}
```

A library with no workflows answers with an empty `<FlowDefs />` rather than an error.
...
- [GetFolderFlows](GetFolderFlows.md) - Get workflow definitions for a specific folder
- [GetFlowDef](GetFlowDef.md) - Get a specific workflow definition with full step details
