...
Turns a library into an archive, so that nothing in it may be changed.

```javascript
await call('ArchiveDomain', { authenticationTicket: ticket, DomainName: 'Finance 2019' });
```

`IsArchive` on [GetDomain](GetDomain.md) is `TRUE` afterwards. **Archiving one that is already an
archive is refused `4000`**, so this is not safe to call blindly - check `IsArchive` first.
[UnarchiveDomain](UnarchiveDomain.md) reverses it, and refuses in the same way.
...
| `4000` | the library is already an archive |
